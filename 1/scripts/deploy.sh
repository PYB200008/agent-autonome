#!/usr/bin/env bash
# =============================================================================
# deploy.sh — Déploiement des sites A et B sur Kubernetes.
...
# --- Vérification de kubectl et du cluster --------------------------------- #
...
if ! kubectl cluster-info &>/dev/null; then
    ...
    echo "        Vérifiez que votre contexte kubectl est correctement configuré."
    exit 1
fi
echo "OK: Cluster accessible"

# --- Vérification de la présence des images locales ------------------------- #
...
echo ""
echo "=== Attente de la mise à jour des Deployments ==="
...
# --- Récupération et affichage de l'adresse du LoadBalancer ----------------- #

echo ""
echo "=== Adresse du LoadBalancer ==="

# Attendre brièvement que l'IP externe soit assignée (timeout 60s)
LB_IP=""
...
if [ -n "${LB_HOST}" ]; then
    ...
    echo "Aucune IP externe assignée (typique en environnement local)."
    echo ""
    echo "Pour accéder au service en environnement local, utilisez :"
